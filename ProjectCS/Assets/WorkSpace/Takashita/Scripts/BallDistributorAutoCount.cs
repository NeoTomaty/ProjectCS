...
// CubicBezierCurve に基づいて、実行時に曲線上に球体を間隔に応じて自動配置する
public class BallDistributorAutoCount : MonoBehaviour
{
    ...
    // 始点と終点だけ大きくしたいサイズ
    public float StartEndSphereScale = 0.4f;
    ...
    private void Start()
    {
        ...
        {
            float t = i / (float)(numberOfBalls - 1);
            Vector3 pos = CalculateBezierPoint(t);
            GameObject ball = Instantiate(SpherePrefab, pos, Quaternion.identity, transform);

            // 始点・終点は大きく、それ以外は通常サイズ
            float scale = (i == 0 || i == numberOfBalls - 1) ? StartEndSphereScale : SphereScale;
