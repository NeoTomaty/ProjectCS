...
public class TestCameraFunction : MonoBehaviour
{
    ...
    private bool IsCameraInterpolating = false; // 通常時のカメラの補完が完了したかどうか
    ...
    private void LateUpdate()
    {
        ...
        if (!isManual && !Input.GetKey(KeyCode.R) && IsPlayerWallHit)
        {
           // float targetYaw = Player.eulerAngles.y;
            ...
            // 補完が終了したかどうかの判定
            bool isYawCompleted = Mathf.Abs(yaw - targetYaw) < 0.01f;  // 目標Yawに十分近いかどうか
            bool isPitchCompleted = Mathf.Abs(pitch - targetPitch) < 0.01f;  // 目標Pitchに十分近いかどうか
