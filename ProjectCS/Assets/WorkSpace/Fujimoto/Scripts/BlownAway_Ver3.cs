...
public class BlownAway_Ver3 : MonoBehaviour
{
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        else if (collision.gameObject.CompareTag("Player"))
        {
            ...
            // プレイヤーとこのオブジェクトのColliderを一時的に衝突無効化
            Collider snackCollider = GetComponent<Collider>();
            Collider playerCollider = collision.collider; // プレイヤーのCollider
    ...
    {
        yield return new WaitForFixedUpdate(); // 物理演算が復帰するまで待つ
        ...
        Debug.Log("Delayed AddForce 完了（FixedUpdate後）");
    }
